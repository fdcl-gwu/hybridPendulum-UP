...
void mexFunction (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    // get MR from matlab
    myComplex* MR = (myComplex*) mymxGetComplex(prhs[2]);

    myComplex* MR_compact = new myComplex[2*size_F.nR_compact];
    modify_u(MR, MR_compact, &size_F);

    // get b from matlab
    myReal* b = mymxGetReal(prhs[3]);

    // get G from matlab
    myReal* G = mymxGetReal(prhs[4]);

    // get dt from matlab
    myReal* dt = mymxGetReal(prhs[5]);

    // get L from matlab
    myReal* L = mymxGetReal(prhs[6]);

    // get u from matlab
    myComplex* u = (myComplex*) mymxGetComplex(prhs[7]);
    ...
    for (int l1 = 0; l1 < size_F.BR; l1++) {
        for (int l2 = 0; l2 < size_F.BR; l2++) {
            ...
            CG[ind_CG] = mymxGetReal(mxGetCell(prhs[8], ind_CG));
        }
    }
    
    // get method from matlab
    char* method;
    method = mxArrayToString(prhs[9]);
    ...
    get_dF(dF1, Fold_compact, X, MR_compact, b, G, L, u_compact, CG, &size_F, size_F_dev);
    ...
    if (stricmp(method,"midpoint") == 0 || stricmp(method,"RK4") == 0) {
        ...
        get_dF(dF2, F2, X, MR_compact, b, G, L, u_compact, CG, &size_F, size_F_dev);
    ...
    if (stricmp(method,"RK4") == 0) {
        ...
        get_dF(dF3, F3, X, MR_compact, b, G, L, u_compact, CG, &size_F, size_F_dev);
        ...
        get_dF(dF4, F4, X, MR_compact, b, G, L, u_compact, CG, &size_F, size_F_dev);
