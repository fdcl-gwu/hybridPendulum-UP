...
__global__ void addup_F(cuComplex* dF, const int nTot)
{
	...
	if (ind1 < nTot) {
		...
		dF[ind1] = cuCaddf(dF[ind1], dF[ind2]);
		dF[ind1] = cuCaddf(dF[ind1], dF[ind3]);
	}
}

__global__ void add_F(cuComplex* dF, const cuComplex* dF_temp, const int nTot)
{
	int ind = threadIdx.x + blockIdx.x*blockDim.x;
	if (ind < nTot)
		dF[ind] = cuCaddf(dF[ind], dF_temp[ind]);
}
...
__global__ void add_FMR_small(cuComplex* dF, const cuComplex* FMR, const int ind_cumR, const Size_F* size_F)
{
	...
	dF[ind_dF] = cuCaddf(dF[ind_dF], FMR[ind_FMR]);
}
...
{
	cuComplex* Fp = new cuComplex[size_F->nTot_compact];
	if (R_first) {
	...
	memcpy(F, Fp, size_F->nTot_compact * sizeof(cuComplex));
	delete[] Fp;
}
	...
	const float* L, const cuComplex* u, const float* const* CG, const Size_F* size_F, const Size_F* size_F_dev)
{
	...
	// set up arrays
	cuComplex* F_dev;
	cudaErrorHandle(cudaMalloc(&F_dev, size_F->nTot_compact*sizeof(cuComplex)));
	cudaErrorHandle(cudaMemcpy(F_dev, F, size_F->nTot_compact*sizeof(cuComplex), cudaMemcpyHostToDevice));

	cuComplex* X_dev;
	cudaErrorHandle(cudaMalloc(&X_dev, 3*size_F->nx*sizeof(cuComplex)));
	cudaErrorHandle(cudaMemcpy(X_dev, X, 3*size_F->nx*sizeof(cuComplex), cudaMemcpyHostToDevice));

	cuComplex* X_ijk_dev;
	cudaErrorHandle(cudaMalloc(&X_ijk_dev, 3*size_F->nx*sizeof(cuComplex)));

	cuComplex* dF3_dev;
	cudaErrorHandle(cudaMalloc(&dF3_dev, 3*size_F->nTot_compact*sizeof(cuComplex)));

	cuComplex* dF_temp_dev;
	cudaErrorHandle(cudaMalloc(&dF_temp_dev, 3*size_F->nR_compact*sizeof(cuComplex)));

	cuComplex* u_dev;
	cudaErrorHandle(cudaMalloc(&u_dev, 3*size_F->nR_compact*sizeof(cuComplex)));
	cudaErrorHandle(cudaMemcpy(u_dev, u, 3*size_F->nR_compact*sizeof(cuComplex), cudaMemcpyHostToDevice));
	...
	cuComplex alpha_cublas = make_cuComplex(1,0);
	cuComplex beta_cublas = make_cuComplex(0,0);
	...
	cuComplex alpha_cutensor = make_cuComplex(0-(float)1/size_F->nx,0);
	cuComplex beta_cutensor = make_cuComplex(0,0);
	...
	for (int i = 0; i < size_F->const_2Bx; i++) {
		for (int j = 0; j < size_F->const_2Bx; j++) {
			for (int k = 0; k < size_F->const_2Bx; k++) {
				...
				for (int n = 0; n < 3; n++) {
					cuComplex* dF3_dev_ijkn = dF3_dev + i*size_F->nR_compact + 
						j*(size_F->nR_compact*size_F->const_2Bx) + k*(size_F->nR_compact*size_F->const_2Bxs) + n*size_F->nTot_compact;
					cuComplex* dF_temp_dev_n = dF_temp_dev + n*size_F->nR_compact;

					cudaErrorHandle(cudaMemcpy(dF3_dev_ijkn, dF_temp_dev_n, size_F->nR_compact*sizeof(cuComplex), cudaMemcpyDeviceToDevice));
				}
			}
		}
	}
	...
	for (int ip = 0; ip < 3; ip++) {
		for (int l = 0; l <= size_F->lmax; l++)
		{
			...
			cublasErrorHandle(cublasCgemm3mStridedBatched(handle_cublas, CUBLAS_OP_N, CUBLAS_OP_T, 2*l+1, 2*l+1, 2*l+1,
	...
	cudaErrorHandle(cudaMemcpy(dF_dev, dF3_dev, size_F->nTot_compact*sizeof(cuComplex), cudaMemcpyDeviceToDevice));
	...
	// set up arrays
	cuComplex* OJO_dev;
	cudaErrorHandle(cudaMalloc(&OJO_dev, 3*size_F->nx*sizeof(cuComplex)));
	cudaErrorHandle(cudaMemcpy(OJO_dev, OJO, 3*size_F->nx*sizeof(cuComplex), cudaMemcpyHostToDevice));

	cuComplex* OJO_ijk_dev;
	cudaErrorHandle(cudaMalloc(&OJO_ijk_dev, 3*size_F->nx*sizeof(cuComplex)));
	...
	for (int i = 0; i < size_F->const_2Bx; i++) {
		for (int j = 0; j < size_F->const_2Bx; j++) {
			for (int k = 0; k < size_F->const_2Bx; k++) {
				...
				for (int ip = 0; ip < 3; ip++) {
					cuComplex* dF3_dev_ijkp = dF3_dev + i*size_F->nR_compact + 
						j*(size_F->nR_compact*size_F->const_2Bx) + k*(size_F->nR_compact*size_F->const_2Bxs) + ip*size_F->nTot_compact;
					cuComplex* dF_temp_dev_p = dF_temp_dev + ip*size_F->nR_compact;

					cudaErrorHandle(cudaMemcpy(dF3_dev_ijkp, dF_temp_dev_p, size_F->nR_compact*sizeof(cuComplex), cudaMemcpyDeviceToDevice));
				}
			}
		}
	}
	...
	// set up arrays
	cuComplex** CG_dev = new cuComplex* [size_F->BR*size_F->BR];
	for (int l1 = 0; l1 <= size_F->lmax; l1++) {
		for (int l2 = 0; l2 <= size_F->lmax; l2++) {
			int m = (2*l1+1)*(2*l2+1);
			int ind_CG = l1+l2*size_F->BR;
			cudaErrorHandle(cudaMalloc(&CG_dev[ind_CG], m*m*sizeof(cuComplex)));
			cudaErrorHandle(cudaMemset(CG_dev[ind_CG], 0, m*m*sizeof(cuComplex)));

			float* CG_dev_d = (float*) CG_dev[ind_CG];
			cudaErrorHandle(cudaMemcpy2D(CG_dev_d, 2*sizeof(float), CG[ind_CG], sizeof(float), sizeof(float), m*m, cudaMemcpyHostToDevice));
		}
	}

	cuComplex** F_strided = new cuComplex* [size_F->BR];
	for (int l = 0; l <= size_F->lmax; l++) {
		int ind = l*(2*l-1)*(2*l+1)/3;
		int m = (2*l+1)*(2*l+1);
		cudaErrorHandle(cudaMalloc(&F_strided[l], m*size_F->nx*sizeof(cuComplex)));
		cudaErrorHandle(cudaMemcpy2D(F_strided[l], m*sizeof(cuComplex), F+ind, size_F->nR_compact*sizeof(cuComplex),
			m*sizeof(cuComplex), size_F->nx, cudaMemcpyHostToDevice));
	}

	cuComplex* MR_dev;
	cudaErrorHandle(cudaMalloc(&MR_dev, 3*size_F->nR_compact*sizeof(cuComplex)));
	cudaErrorHandle(cudaMemcpy(MR_dev, MR, 3*size_F->nR_compact*sizeof(cuComplex), cudaMemcpyHostToDevice));

	cuComplex* FMR_dev;
	int m = (2*size_F->lmax+1) * (2*size_F->lmax+1);
	cudaErrorHandle(cudaMalloc(&FMR_dev, 3*m*sizeof(cuComplex)));

	cuComplex* FMR_temp_dev;
	cudaErrorHandle(cudaMalloc(&FMR_temp_dev, 3*size_F->nx*sizeof(cuComplex)));

	cudaErrorHandle(cudaMemset(dF3_dev, 0, 3*size_F->nTot_compact*sizeof(cuComplex)));

	// get c
	float* c = new float[size_F->const_2Bx];
	for (int i = 0; i < size_F->const_2Bx; i++) {
		deriv_x(&c[i], i, size_F->Bx, *L);
	}

	float* c_dev;
	cudaErrorHandle(cudaMalloc(&c_dev, size_F->const_2Bx*sizeof(float)));
	cudaErrorHandle(cudaMemcpy(c_dev, c, size_F->const_2Bx*sizeof(float), cudaMemcpyHostToDevice));
		...
		for (int l1 = 0; l1 <= size_F->lmax; l1++) {
			for (int l2 = 0; l2 <= size_F->lmax; l2++) {
				if (abs(l1-l2)<=l && l1+l2>=l) {
					...
					alpha_cutensor.x = (float) -l12/(2*l+1);
					...
					for (int m = -l; m <= l; m++) {
						...
						for (int n = -l; n <= l; n++) {
							...
							cublasErrorHandle(cublasCgemm3mStridedBatched(handle_cublas, CUBLAS_OP_T, CUBLAS_OP_N, 2*l1+1, 2*l2+1, 2*l2+1,
							...
							cublasErrorHandle(cublasCgemm3mStridedBatched(handle_cublas, CUBLAS_OP_N, CUBLAS_OP_N, 2*l1+1, 2*l1+1, 2*l2+1,
	...
	// return
	cudaErrorHandle(cudaMemcpy(dF, dF_dev, size_F->nTot_compact*sizeof(cuComplex), cudaMemcpyDeviceToHost));
	...
	const float* L, const cuComplex* u, const float* const* CG, const Size_F* size_F, const Size_F* size_F_dev)
{
	...
	if (size_F->nx*size_F->nR_split > size_F->nTot_splitx) {
		cudaErrorHandle(cudaMalloc(&F_dev, size_F->nx*size_F->nR_split*sizeof(cuComplex)));
	} else {
		cudaErrorHandle(cudaMalloc(&F_dev, size_F->nTot_splitx*sizeof(cuComplex)));
	}
	...
	cuComplex* dF3_dev;
	cudaErrorHandle(cudaMalloc(&dF3_dev, 3*size_F->nTot_splitx*sizeof(cuComplex)));
	...
	cuComplex* dF3 = new cuComplex[3*size_F->nTot_compact];
	...
	cuComplex alpha_cutensor = make_cuComplex(-(float)1/size_F->nx,0);
	cuComplex beta_cutensor = make_cuComplex(0,0);
	...
	for (int is = 0; is < size_F->ns; is++) {
		...
		cudaErrorHandle(cudaMemcpy(F_dev, F+is*size_F->nx*size_F->nR_split, size_F->nx*nR_split*sizeof(cuComplex), cudaMemcpyHostToDevice));
			...
			for (int j = 0; j < size_F->const_2Bx; j++) {
				for (int k = 0; k < size_F->const_2Bx; k++) {
					...
					for (int ip = 0; ip < 3; ip++) {
						...
						cudaMemcpy(dF3+ind_dF3, dF3_dev+ip*nR_split, nR_split*sizeof(cuComplex), cudaMemcpyDeviceToHost);
					}
				}
			}
		}
	...
	for (int k = 0; k < size_F->const_2Bx; k++) {
		for (int ip = 0; ip < 3; ip++) {
			...
			cudaErrorHandle(cudaMemcpy(F_dev, dF3+ind_dF3, size_F->nTot_splitx*sizeof(cuComplex), cudaMemcpyHostToDevice));
		...
		cudaErrorHandle(cudaMemcpy(dF+ind_dF, dF3_dev, size_F->nTot_splitx*sizeof(cuComplex), cudaMemcpyDeviceToHost));
	}
	...
	for (int k = 0; k < size_F->const_2Bx; k++) {
		for (int ip = 0; ip < 3; ip++) {
			...
			cudaErrorHandle(cudaMemcpy(dF3_dev+ind_dF3_dev, dF3+ind_dF3, size_F->nTot_splitx*sizeof(cuComplex), cudaMemcpyHostToDevice));
		}
		...
		int ind_dF = k*size_F->nTot_splitx;
		cudaErrorHandle(cudaMemcpy(dF3_dev+size_F->nTot_splitx, dF+ind_dF, size_F->nTot_splitx*sizeof(cuComplex), cudaMemcpyHostToDevice));

		add_F <<<gridsize_512_nTot, blocksize_512_nTot>>> (dF3_dev+size_F->nTot_splitx, dF3_dev, size_F->nTot_splitx);
		cudaErrorHandle(cudaGetLastError());

		cudaErrorHandle(cudaMemcpy(dF+ind_dF, dF3_dev+size_F->nTot_splitx, size_F->nTot_splitx*sizeof(cuComplex), cudaMemcpyDeviceToHost));
	}
	...
	cuComplex* FMR_temp_dev;
	cudaErrorHandle(cudaMalloc(&FMR_temp_dev, 3*size_F->const_2Bxs*sizeof(cuComplex)));

	cuComplex** F_strided = new cuComplex* [size_F->BR];
	for (int l = 0; l <= size_F->lmax; l++) {
		int m = (2*l+1)*(2*l+1);
		cudaErrorHandle(cudaMalloc(&F_strided[l], m*size_F->const_2Bxs*sizeof(cuComplex)));
	}
	...
	for (int k = 0; k < size_F->const_2Bx; k++) {
		...
		for (int l = 0; l <= size_F->lmax; l++) {
			int ind = l*(2*l-1)*(2*l+1)/3;
			int m = (2*l+1)*(2*l+1);
			cudaErrorHandle(cudaMemcpy2D(F_strided[l], m*sizeof(cuComplex), F+ind_Fold+ind, size_F->nR_compact*sizeof(cuComplex),
				m*sizeof(cuComplex), size_F->const_2Bxs, cudaMemcpyHostToDevice));
		}

		cudaErrorHandle(cudaMemset(dF3_dev, 0, 3*size_F->nTot_splitx*sizeof(cuComplex)));
		...
		for (int ip = 0; ip < 3; ip++) {
			...
			int ind_dF3_dev = ip*size_F->nTot_splitx;
			cudaErrorHandle(cudaMemcpy(dF3+ind_dF3, dF3_dev+ind_dF3_dev, size_F->nTot_splitx*sizeof(cuComplex), cudaMemcpyDeviceToHost));
		}
	}
